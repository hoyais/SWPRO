...
class UserSolution {
    ...
    void addRail(int mFront, int mDirection) {
        ...
        if (isDebug) System.out.println(nRail);

        if (isDebug) {
            if (mFront == 1)    printRailList(fList);
            else printRailList(rList);
        }

    }

    public void printRailList(LinkedList<Rail> list) {
    ...
    public int delRail(int mRow, int mCol) {
        ...
        if (map.get(hashKey(mRow, mCol)) == null || map.get(hashKey(mRow, mCol)) == 0
            ...
            if (isDebug2) System.out.println("delRail 0 delCount = " + delCount);
            return delCount;
        }
        ...
        if (isDebug2) System.out.println("delRail delCount = " + delCount);
        return delCount;
    }
    ...
    public int deleteRailList(int mFront, LinkedList<Rail> railList, int mRow, int mCol) {
        ...
        if (isDebug) System.out.println("IsCircle : " + isCircle + " mFront : " + mFront + " mRow nCol : " + mRow + ", " + mCol);
        ...
        if (isInRail) {
            if (!isCircle) {
                ...
                for (int i = 1; i < railSize; i++) {
                    ...
                    if (isDebug) System.out.println("Delete : " + rail);
    ...
    public boolean circularCheck() {
        ...
        if (isDebug) {
            System.out.println("rowGap = " + rowGap + " colGap = " + colGap);
            System.out.println("fRail : " + fRail);
            System.out.println("rRail : " + rRail);
        }
